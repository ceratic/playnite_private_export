...
{
    public class MyLibExport : GenericPlugin
    {
        ...
        private MyLibExportSettingsViewModel settings { get; set; }
        public override Guid Id { get; } = Guid.Parse("62d53f25-4e62-4f27-a49e-89e73cb1fd48");

        public MyLibExport(IPlayniteAPI api) : base(api)
        {
            settings = new MyLibExportSettingsViewModel(this, api);
            dialogs = api.Dialogs;
        ...
        public override IEnumerable<MainMenuItem> GetMainMenuItems(GetMainMenuItemsArgs args)
        {
            ...
            {
                Description = "Resynchronize library with MyLibExport",
        ...
        public override IEnumerable<GameMenuItem> GetGameMenuItems(GetGameMenuItemsArgs args)
        {
            return new List<GameMenuItem>
            {
                new GameMenuItem
                {
                    Description = "Synchronize game with MyLibExport",
                    Action = a =>
                    {
        ...
        {
            var clientApi = new MyLibExportAccountClient(this, PlayniteApi);
            var scanRes = dialogs.ActivateGlobalProgress((args) =>
                {
                    clientApi.SynchroniseGames(args).GetAwaiter().GetResult();
                },
                new GlobalProgressOptions("Kicking off a full MyLibExport resynchronisation.", true)
                {
        ...
        {
            var clientApi = new MyLibExportAccountClient(this, PlayniteApi);
            if (LoadPluginSettings<MyLibExportSettings>().showProgress || overrideShowProgress)
            {
                var scanRes = dialogs.ActivateGlobalProgress((args) =>
                {
                    clientApi.UpdateGames(args, games).GetAwaiter().GetResult();
                },
                    new GlobalProgressOptions("Kicking off a MyLibExport games update.", true)
                    {
                        IsIndeterminate = true
                    }
                );
        ...
        {
            if (this.LoadPluginSettings<MyLibExportSettings>().keepInSync)
            {
        ...
        {
            return new MyLibExportSettingsView();
        }
    }
}